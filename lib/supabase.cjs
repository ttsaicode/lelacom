...
async function updateAdSettings(updates) {
  ...
  if (isConfigured && supabase) {
    try {
      await supabase.from("system_settings").upsert({ key: "ad_settings", value: merged });
    } catch (err) {}
  }
...
async function uploadAdMedia(localFilePath, originalName, mimeType) {
  if (!isConfigured || !supabase) return null;
  const fs = require("fs");
  const path = require("path");
  const crypto = require("crypto");
  const bucket = process.env.SUPABASE_ADS_BUCKET || "ad-media";
  const ext = (path.extname(originalName || "").toLowerCase() || ".bin").replace(/[^.a-z0-9]/g, "");
  const objectName = `ads/${crypto.randomBytes(16).toString("hex")}${ext}`;
  try {
    try { await supabase.storage.createBucket(bucket, { public: true, fileSizeLimit: "50MB" }); } catch (_) {}
    ...
    if (error) { console.warn("[SUPABASE] uploadAdMedia:", error.message); return null; }
    const { data } = supabase.storage.from(bucket).getPublicUrl(objectName);
    return data?.publicUrl || null;
  } catch (err) {
    console.warn("[SUPABASE] uploadAdMedia exception:", err.message);
    return null;
  }
}
